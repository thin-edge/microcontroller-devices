...
- [x] 1.1 `tedge-zephyr/src/tedge_remote_access.c`: parse `530`, resolve the target, check the policy and the application hook, take a seat (D2, D3)
- [x] 1.2 TCP connect to the target, then the WebSocket to `wss://<tenant>/service/remoteaccess/device/<key>` with `Sec-WebSocket-Protocol: binary` and the Bearer token; never log the key
- [x] 1.3 The bridge loop: two static buffers, one frame per read, idle timeout, close reasons, byte counters (D4)
- [x] 1.4 Telnet echo offer for port 23 (D5)
- [x] 1.5 Session pool and cap: `TEDGE_REMOTE_ACCESS_MAX_SESSIONS` threads and buffers; a request over the cap fails immediately with the limit in the reason
- [x] 1.6 Results to the client thread through a message queue; the dispatcher in `tedge_c8y.c` sends `501`/`503`/`502` and the open/close events
- [x] 1.7 `tedge_RemoteAccess` twin data through the core's twin store, on connect and on every session change; `TEDGE_REMOTE_ACCESS_TWIN_SESSIONS` controls the target list
- [x] 1.8 Kconfig: `TEDGE_REMOTE_ACCESS` selects `TEDGE_HTTP`, `WEBSOCKET_CLIENT` and `PSA_WANT_ALG_SHA_1`; add `TEDGE_REMOTE_ACCESS_STACK_SIZE` and `TEDGE_REMOTE_ACCESS_TWIN_SESSIONS`; drop the experimental gate
