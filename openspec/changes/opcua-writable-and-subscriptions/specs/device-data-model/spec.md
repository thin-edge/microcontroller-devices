...
The address space SHALL expose writable control nodes under the Device object: a
numeric `setpoint` (Double) and a boolean `enabled`, each with read and write
access. Their values are held in RAM (not persisted across reboot).

#### Scenario: Setpoint is writable and reads back
- **WHEN** a client writes a value to the `setpoint` node
- **THEN** the write SHALL succeed and reading `setpoint` SHALL return the written value

#### Scenario: Enabled flag is writable
- **WHEN** a client writes true or false to the `enabled` node
- **THEN** the write SHALL succeed and reading `enabled` SHALL return the written value
...
- **WHEN** a client writes `setpoint` or `enabled`
