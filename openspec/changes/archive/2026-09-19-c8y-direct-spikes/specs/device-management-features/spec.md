...
TLS sessions than the board's measured heap supports.

#### Scenario: Constrained board

- **WHEN** the ESP32-WROOM-32 OPC-UA application is built with its default
  profile
- **THEN** the profile excludes the features whose measured cost exceeds the
  image's free heap, and the image boots and serves OPC-UA as before
